...
int book_code[MAX_BOOKS];          // changed from book_isbn to book_code
...
int main() {
    ...
    for (int i = 0; i < MAX_BOOKS; i++) {
        ...
        book_code[i] = i + 1000;   // initialize with book code
    ...
    while (true) {
        ...
        if (password == "potato") { // updated password
            if (is_admin) {
...
void issue_item(int roll) {
    ...
    for (int i = 0; i < book_count; i++) {
        if (book_available[i]) {
            cout << i + 1 << ". " << book_title[i] << " by " << book_author[i] << " (Code: " << book_code[i] << ")\n";
        }
    }
...
void add_book() {
    ...
    if (find_book(code) != -1) {
        cout << "A book with this code already exists.\n";
        return;
    }

    book_code[book_count] = code;
